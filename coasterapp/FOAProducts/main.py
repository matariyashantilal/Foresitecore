...
product_file = "/Users/mac/Documents/Projects/foresitecore-django/app/FOAProducts/products.txt"
...
def add(sku_list):
    ...
    for sku in sku_list:
        try:
            ...
            id = shopify_helper_new.addProduct(product)
            if id == -1:
...
def updateInventory():
    ...
    for index, shopify_id in enumerate(product_list):
        shopify_helper_new.updateInventory(shopify_id, result[index]["qty"])
    return "FOA inventory updated (" + result.count() + " products)"
