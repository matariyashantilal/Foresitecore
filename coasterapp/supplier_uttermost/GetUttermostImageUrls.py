...
def scrape_uttermost_product_images(link):
...
def cache_uttermost_images_links():
    ...
    cached_json_file = os.path.join(settings.CACHE_BACKUP_DIR, thejsonfilename)
    if os.path.exists(cached_json_file):
        ...
        with open(thefulljsonpath, 'w+') as f:
            ...
            for cat_link in cat_links:
                for links in get_links(cat_link):
                    for link in links:
                        if link not in data['urls']:
                            dictx = scrape_uttermost_product_images(link)
                            if dictx:
