...
from coasterapp.supplier_uttermost.GetUttermostImageUrls import scrape_uttermost_product_images
...
class ManageUttermostProducts:
    ...
    def scrape_images_for_product(self, productID, newJsonObj):
        ...
        if r.ok:
            ...
            if url:
                ...
                dictx = scrape_uttermost_product_images(str(url))
                if dictx:
    ...
    def loadUttermostCatalog(self, catalogfile, vendor):
        ...
        for row in rows:
            ...
            thefulljsonpath = os.path.join(settings.UTTERMOST_TEMP_DIR, thejsonfilename)
            if os.path.exists(thefulljsonpath):
                ...
                self.scrape_images_for_product(
                    str(row["Item Number"]), newJsonObj)
            shopify_helper_new.upsertShopifyProduct(
